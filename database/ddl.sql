...
begin
insert into inventory_table (shoe_id) values (NEW.shoe_id);
end;
...
CREATE PROCEDURE get_colorway(in color varchar(255))
BEGIN
    SELECT DISTINCT shoe_table.*
    FROM shoe_table
    right join inventory_table on shoe_table.shoe_id = inventory_table.shoe_id 
    where shoe_table.shoe_colorway like concat('%',color,'%');
END;

CREATE PROCEDURE get_Name(in names varchar(255))
BEGIN
    SELECT DISTINCT shoe_table.*
    FROM shoe_table
    right join inventory_table on shoe_table.shoe_id = inventory_table.shoe_id 
    where shoe_table.shoe_name like concat('%',names,'%');
END;


CREATE PROCEDURE get_Brand(in brand varchar(255))
BEGIN
    select DISTINCT shoe_table.* 
    from shoe_table 
    right join inventory_table on shoe_table.shoe_id = inventory_table.shoe_id 
    where shoe_table.shoe_brand = concat('%',brand,'%');
END;
